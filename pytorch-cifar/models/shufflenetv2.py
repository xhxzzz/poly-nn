...
def count(t, label):
    ...
    with open('shufflenetv2-stat.txt', 'a') as f:
        f.write(f'{label}\n')
        f.write(f'>= 10: {ge10}\n')
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        count(out, 'before first relu in basicblock')
        out = F.relu(out)
        out = self.bn2(self.conv2(out))
        out = self.bn3(self.conv3(out))
        count(out, 'before second relu in basicblock')
        out = F.relu(out)
...
class DownBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        count(out1, 'before left relu')
        out1 = F.relu(out1)
        # right
        out2 = self.bn3(self.conv3(x))
        count(out2, 'before first right relu')
        out2 = F.relu(out2)
        out2 = self.bn4(self.conv4(out2))
        out2 = self.bn5(self.conv5(out2))
        count(out2, 'before second right relu')
        out2 = F.relu(out2)
        # concat
...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        count(out, 'before first relu in shufflenetv2')
        out = F.relu(out)
        ...
        out = self.layer3(out)
        out = self.bn2(self.conv2(out))
        count(out, 'before second relu in shufflenetv2')
        out = F.relu(out)
